...
def shiftedUp = generator().observeOn(Schedulers.io()).map(shiftUp).debug("Shifted Up")
def shiftedDown = shiftedUp.observeOn(Schedulers.computation()).map(shiftDown).debug("Shifted Down")
